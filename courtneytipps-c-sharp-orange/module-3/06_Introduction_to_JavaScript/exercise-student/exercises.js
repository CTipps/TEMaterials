...
*/
		function seeColor(color) {
			let redCheck = color.startsWith("red", 0)
			let blueCheck = color.startsWith("blue", 0)
			if (redCheck) {
				return "red";
			} else if (blueCheck) {
				return "blue";
...
*/
			function frontAgain(x) {
				if (x[0] + x[1] == x[x.length-2] + x[x.length-1]) {
					return true;
				} else {
					return false;
				}
